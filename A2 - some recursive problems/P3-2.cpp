...
void changeLargest(node* newPath, int pathLengh, vector<point>& largestPathSlot){

    largestPathSlot.clear();

    node* currentNode = newPath;

    for( int i = 1; i < pathLengh; i++){

        largestPathSlot.push_back( newPoint(currentNode) );
        currentNode = currentNode->Previous;
    }
}

//***********************       main recursive functions
void findLargerPath(cityMap& map, node* currentPosition, visitedPoint* lastVisitedNode, vector<point>& lastLargest, int pathLengh){
    ...
    for( int move = 0; move < MAX_MOVES; move++ ){
        ...
        if(isPosibleMove(map, currentPosition, new_X, new_Y, lastVisitedNode)){
            ...
            cantMoveAnymore = false;
            node* nextPoint = setNode(new_X, new_Y, currentPosition);
   
            findLargerPath(map, nextPoint, lastVisitedNode, lastLargest, pathLengh);
        }
    }

    free(lastVisitedNode);

    if( cantMoveAnymore && ( lastLargest.size() < pathLengh ) ){

        changeLargest(currentPosition, pathLengh, lastLargest);
    }

    free(currentPosition);
}

vector<point> findLargestPath(cityMap& map, const int MAX_X, const int MAX_Y){
    ...
    for(int i = 0; i < MAX_Y; i++){
        for(int j = 0; j < MAX_X; j++){
            ...
            findLargerPath(map, newNode, nullptr, largestPath, 0);
        }
    }
...
int main(){
    ...
    vector<point> answer = findLargestPath(map, MAX_X, MAX_Y);
